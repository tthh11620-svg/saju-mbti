...
// 5. 시스템 프롬프트
// =====================================================================
const COMBINED_SYSTEM_PROMPT = `[역할]
...
2. 구조와 흐름 파악: 오행의 개수보다 일간과의 관계, 지지의 합/충, 상호작용이 만드는 '행동 양식'을 우선하여 분석한다.
3. 행동 기반 번역: 추상적 자연물 비유를 버리고, 구체적인 일상 행동 패턴으로 설명하라.

[MBTI 축별 해석 가이드라인]
- N/S: "정보 처리 방식"이다. 토가 많다고 무조건 S가 아니다. 충/합이 많으면 N 성향이 나타난다.
- T/F: "판단 기준"이다. 편관은 '논리'가 아니라 '외부 기준 수용'이다. 금+관성+토 → T, 수+식상+목 → F
- J/P: "행동 방식"이다. 충이 많다고 계획적(J)인 게 절대 아니다. 충 많음 → P다.
